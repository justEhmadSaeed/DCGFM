...
class SubgraphNopromptDataset(SubgraphHierDataset):
    def __getitem__(self, index):
        (feat, edge_feat, edge_index, e_type, target_node_id, emb, label, binary_rep,) = self.make_feature_graph(index)
        new_subg = pyg.data.Data(feat, edge_index, y=label, edge_attr=edge_feat, edge_type=e_type)
        return new_subg
...
class SubgraphKGNopromptDataset(SubgraphKGHierDataset):
    def __getitem__(self, index):
        (feat, edge_feat, edge_index, e_type, target_node_id, embs, label, binary_rep,) = self.make_feature_graph(index)
        new_subg = pyg.data.Data(feat, edge_index, y=label, edge_attr=edge_feat, edge_type=e_type)
        return new_subg 
...
class GraphListNopromptDataset(GraphListHierDataset):
    def __getitem__(self, index):
        (feat, edge_feat, edge_index, e_type, target_node_id, emb, label, binary_rep) = self.make_feature_graph(index)
        new_subg = pyg.data.Data(feat, edge_index, y=label, edge_attr=edge_feat, edge_type=e_type)
        return new_subg
...
class FewShotDataset_fixed_hard(DatasetWithCollate): 
    def __init__(self, fsmanager, query_graph_dataset, support_graph_dataset, fs_edge_feats, task_level, fs_sample_size, cache_dir='./cache_data/ofa_dataset'):
        """
        FewShotDataset: use data indices generated from fsmanager to index int
        query_graph_dataset/support_graph_dataset (GraphTextDataset) to get query and support prompted graph only
        with NOI prompt node. It them assembles the graphs to a few-shot in-context prompted graphs.
        Args:
            fsmanager: query and support data indices manager
        ...
        self.llm_tokenizer = None
        self.llm_max_length = None
        self.fs_sample_size = fs_sample_size
        ...
        self.node_ids_cache_path = os.path.join(cache_dir, f'node_ids_{self.fs_sample_size}_{self.task_level}.pkl')
        self.class_ind_cache_path = os.path.join(cache_dir, f'class_ind_{self.fs_sample_size}_{self.task_level}.pkl')
        self.qry_ind_cache_path = os.path.join(cache_dir, f'qry_ind_{self.fs_sample_size}_{self.task_level}.pkl')

        self.all_node_ids, self.all_class_ind, self.all_qry_ind = self.load_or_generate_indices()
        self.all_no_prompt_data = self.generate_no_prompt_data()
        
        self.effective_indices = range(self.fs_sample_size)
    
    def generate_no_prompt_data(self):
        current_data = []
        for i in range(self.fs_sample_size):
            current_data.append(self.get_sample_without_prompt(i))
        return current_data
    
    def get_sample_without_prompt(self, index):
        node_ids, class_ind = self.all_node_ids[index], self.all_class_ind[index]
        qry_ind = self.all_qry_ind[index]
        qry_ind_num = node_ids[qry_ind.view(-1), 0] 
        qry_feature_graph = self.get_feature_graph(self.query_graph_dataset, qry_ind_num)
        return qry_feature_graph

    def check_size(self, data, cache_path=None):
        if cache_path is None:
            with open("debug_bigdata.txt", "a") as f:
                f.write(f"the memory size of no promptdata: {asizeof.asizeof(data)}\n")
        else:
            pickle_size = len(pickle.dumps(data))
            with open("debug_bigdata.txt", "a") as f:
                f.write(f"the pickle size of {cache_path}: {pickle_size}\n")
                
    def load_or_generate_indices(self):
        if os.path.exists(self.node_ids_cache_path) and os.path.exists(self.class_ind_cache_path) and os.path.exists(self.qry_ind_cache_path):
            print(f"Loading cached indices from {self.node_ids_cache_path}, {self.class_ind_cache_path}, {self.qry_ind_cache_path}")
            return pickle.load(open(self.node_ids_cache_path, 'rb')), pickle.load(open(self.class_ind_cache_path, 'rb')), pickle.load(open(self.qry_ind_cache_path, 'rb'))
        else:
            print(f"Generating new indices for {self.node_ids_cache_path}, {self.class_ind_cache_path}, {self.qry_ind_cache_path}")
            all_node_ids, all_class_ind, all_qry_ind = self.generate_all_indices()
            with open(self.node_ids_cache_path, 'wb') as f:
                pickle.dump(all_node_ids, f)
            with open(self.class_ind_cache_path, 'wb') as f:
                pickle.dump(all_class_ind, f)
            with open(self.qry_ind_cache_path, 'wb') as f:
                pickle.dump(all_qry_ind, f)
            print("Cached indices")
            
            self.check_size(all_node_ids, self.node_ids_cache_path)
            self.check_size(all_class_ind, self.class_ind_cache_path)
            self.check_size(all_qry_ind, self.qry_ind_cache_path)
            return all_node_ids, all_class_ind, all_qry_ind
    
    def generate_all_indices(self):
        all_node_ids, all_class_ind, all_qry_ind = [], [], []
        for i in range(self.fs_sample_size):
            node_ids, class_ind = self.fs_idx_loader.get_few_shot_idx()
            qry_ind = torch.randint(len(class_ind), (1, 1))
            all_node_ids.append(node_ids)
            all_class_ind.append(class_ind)
            all_qry_ind.append(qry_ind)
        return all_node_ids, all_class_ind, all_qry_ind

    def __getitem__(self, index): 
        real_index = self.effective_indices[index] 
        return self.get_sample(real_index)
    ...
    def get_feature_graph(self, dataset: GraphTextDataset, index):
        
        (feat, edge_feat, edge_index, e_type, target_node_id, embs, label, binary_rep,) = dataset.make_feature_graph(index)
        
        feat = torch.tensor(feat) 
        
        new_subg = pyg.data.Data(x = feat, edge_index = edge_index) 
        return new_subg
...
class InfoBatchMultiDataset(DatasetWithCollate):
    def __init__(self, datas, num_epochs: int,
                    prune_ratio: float = 0.5, delta: float = 0.875, hard_pruning_ratio=0.0, hard_pruning_epochs=25):
        self.datas = datas
        self.sizes = np.array([len(d) for d in datas])
        self.compute_sizes()
        ...
        self.delta = delta
        self.scores = torch.ones(self.dataset_size) * 3
        self.weights = torch.ones(self.dataset_size)
        
        self.cur_batch_index = None
        self.hard_pruning_ratio = hard_pruning_ratio
        self.hard_pruning_epochs = hard_pruning_epochs

    def compute_sizes(self):
        
        self.dataset_size = np.sum(self.sizes) 
        self.size_seg = np.cumsum(self.sizes) 
        self.ind2dataset = np.arange(len(self.datas)).repeat(self.sizes) 
    ...
    def __getitem__(self, index):
        dataset_ind = self.ind2dataset[index] 
        ret_data = self.datas[dataset_ind][index - self.data_start_index[dataset_ind]] 
        return index, ret_data
    """
    def compute_sizes(self):
        self.aug_sizes = (self.sizes * np.array(self.dataset_multiple)).astype(int)
        self.size_seg = np.cumsum(self.aug_sizes)
    ...
    def update(self, values, cur_batch_indices): 
        
        assert isinstance(values, torch.Tensor)
        
        batch_size = values.shape[0]
        assert len(cur_batch_indices) == batch_size, 'not enough index'
        
        device = values.device
        
        weights = self.weights[cur_batch_indices.cpu().long()].to(device)
        cur_batch_indices = cur_batch_indices.to(device)
        
        loss_val = values.detach().clone()
        
        
        if dist.is_available() and dist.is_initialized():
            iv = torch.cat([cur_batch_indices.view(1, -1), loss_val.view(1, -1)], dim=0)
            iv_whole_group = concat_all_gather(iv, 1) 
            cur_batch_indices = iv_whole_group[0]
            loss_val = iv_whole_group[1]
        self.scores[cur_batch_indices.cpu().long()] = loss_val.cpu() 
        
        
        weighted_value = values * weights
        values_mean = weighted_value.mean()
        return values_mean 
    ...
    @property
    def stop_prune(self):
        return int(self.num_epochs * self.delta)

    def reset_weights(self):
        self.weights[:] = 1
    
    def get_collate_fn(self):
        original_collate = self.datas[0].get_collate_fn()        
        def custom_collate(batch): 
            try:
                indices = [item[0] for item in batch]
                data = [item[1] for item in batch]
                self.set_active_indices(torch.tensor(indices))
                collated_data = original_collate(data)
                new_data = {
                    'data': collated_data,
                    'indices': self.cur_batch_index
                }
                return new_data
...
class IBSampler(object):
    def __init__(self, dataset: InfoBatchMultiDataset): 
        self.dataset = dataset
        self.stop_prune = dataset.stop_prune 
        self.iterations = -1 
        self.sample_indices = None 
        self.iter_obj = None 
        self.prune_ratio = 1.0 - self.dataset.keep_ratio
        self.delta = self.dataset.delta
        self.hard_pruning_ratio = dataset.hard_pruning_ratio
        self.hard_pruning_epochs = dataset.hard_pruning_epochs
        
        visible_devices = os.environ.get('CUDA_VISIBLE_DEVICES', '')
        if visible_devices:
            self.num_visible_gpus = len(visible_devices.split(','))
        else:
            self.num_visible_gpus = torch.cuda.device_count()
        self.reset() 
    ...
    def reset(self):
        ...
        tmp_iterations = self.iterations - 2 * (self.num_visible_gpus - 1)
        
        if tmp_iterations > self.stop_prune: 
        ...
        print(f"the size of dataset in epoch {tmp_iterations} is {len(self.sample_indices)}")       
