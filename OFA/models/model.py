...
class LLMModel(torch.nn.Module):
    ...
    def get_llm_model(self):
        # 把huggingface的模型名称替换成了model_path
        if self.llm_name == "llama2_7b":
            model_path = os.path.join(self.cache_dir, "models--meta-llama--Llama-2-7b-hf")
            ModelClass = LlamaForCausalLM
            TokenizerClass = LlamaTokenizer

        elif self.llm_name == "llama2_13b":
            model_path = os.path.join(self.cache_dir, "models--meta-llama--Llama-2-13b-hf")
            ModelClass = LlamaForCausalLM
            TokenizerClass = LlamaTokenizer

        elif self.llm_name == "e5":
            model_path = os.path.join(self.cache_dir, "models--intfloat--e5-large-v2")
            ModelClass = AutoModel
            TokenizerClass = AutoTokenizer

        elif self.llm_name == "BERT":
            model_path = os.path.join(self.cache_dir, "models--google-bert--bert-base-uncased")
            ModelClass = AutoModel
            TokenizerClass = AutoTokenizer
