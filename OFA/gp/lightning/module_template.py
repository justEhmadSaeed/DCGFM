...
class IBBaseTemplate(LightningModule):
    def __init__(
        self,
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        score, loss = self.compute_results(
            data, batch_idx, self.exp_config.train_state_name[dataloader_idx]
        )

        device = loss.device

        true_nodes_idx = torch.where(data.true_nodes_mask)[0]  
        graph_losses = torch.zeros(data.num_graphs, device=device)
        num_nodes = torch.zeros(data.num_graphs, device=device)

        
        graph_losses = torch.scatter_add(graph_losses, 0, data.batch[true_nodes_idx], loss)
        num_nodes = torch.scatter_add(num_nodes, 0, data.batch[true_nodes_idx], 
                                    torch.ones_like(true_nodes_idx, device=device, dtype=torch.float))

        
        graph_losses = graph_losses / num_nodes.clamp(min=1)
        
        
        ib_loss = self.info_batch.update(graph_losses, indices)
