...
from ofa_datasets_combine import (MultiDataset, GraphListHierDataset, SubgraphHierDataset,
                          SubgraphLinkHierDataset, SubgraphKGHierDataset, SubgraphNopromptDataset,
                          GraphListNopromptDataset, SubgraphNopromptLinkDataset, SubgraphKGNopromptDataset, FewShotDataset, FewShotDataset_fixed_hard)
...
no_prompt_dict = {"ConstructNodeCls": "ConstructNodeNopromptCls", "ConstructLinkCls": "ConstructLinkNopromptCls", "ConstructKG": "ConstructKGNoprompt", "ConstructMolCls": "ConstructMolNopromptCls"}


from ofa_datasets_combine import InfoBatchMultiDataset
...
def ConstructNodeNopromptCls(dataset, split, split_name, prompt_feats, to_bin_cls_func, global_data, task_level, **kwargs):
    text_g = dataset.data

    return SubgraphNopromptDataset(text_g, prompt_feats["class_node_text_feat"], prompt_feats["prompt_edge_text_feat"],
                               prompt_feats["noi_node_text_feat"], split[split_name], to_undirected=True,
                               process_label_func=to_bin_cls_func, prompt_edge_list=dataset.get_edge_list(task_level),
                               **kwargs, )
...
def ConstructLinkNopromptCls(dataset, split, split_name, prompt_feats, to_bin_cls_func, global_data, task_level, **kwargs):
    text_g = dataset.data
    edges = text_g.edge_index
    train_graph = global_data

    return SubgraphNopromptLinkDataset(train_graph, prompt_feats["class_node_text_feat"],
                                   prompt_feats["prompt_edge_text_feat"], prompt_feats["noi_node_text_feat"],
                                   edges.T[split[split_name]].numpy(), to_undirected=True, hop=3,
                                   process_label_func=to_bin_cls_func, prompt_edge_list=dataset.get_edge_list(task_level),
                                   **kwargs, )
...
def ConstructKGNoprompt(dataset, split, split_name, prompt_feats, to_bin_cls_func, task_level, global_data, **kwargs):
    edge_data = [global_data[0][split[split_name]].tolist(), global_data[1][split[split_name]].tolist()]
    ...
    return SubgraphKGNopromptDataset(global_data[-1], prompt_feats["class_node_text_feat"],
                                 prompt_feats["prompt_edge_text_feat"], prompt_feats["noi_node_text_feat"], edge_data,
                                 to_undirected=True, hop=2, process_label_func=to_bin_cls_func,
                                 prompt_edge_list=dataset.get_edge_list(task_level), fs_edges=fs_edges, **kwargs, )
...
def ConstructMolNopromptCls(dataset, split, split_name, prompt_feats, to_bin_cls_func, task_level, global_data, **kwargs):
    return GraphListNopromptDataset(dataset, prompt_feats["class_node_text_feat"], prompt_feats["prompt_edge_text_feat"],
                                prompt_feats["noi_node_text_feat"], split[split_name],
                                process_label_func=to_bin_cls_func, prompt_edge_list=dataset.get_edge_list(task_level),
                                **kwargs, )
...
def ConstructFSTask(dataset, split, split_name, prompt_feats, to_bin_cls_func, global_data, task_level, **kwargs):
    ...
    support_graph_dataset = globals()[kwargs["base_construct"]](dataset=dataset, split=pseudo_split, split_name="pseudo", 
                                                                prompt_feats=prompt_feats, to_bin_cls_func=to_bin_cls_func, 
                                                                global_data=global_data,task_level=task_level, fs_flag=split_name, **kwargs)
...
def ConstructFSTask_fixed_hard(dataset, split, split_name, prompt_feats, to_bin_cls_func, global_data, task_level, fs_sample_size, **kwargs):
    original_idx = np.concatenate(split[split_name][1])
    pseudo_split = {"pseudo": original_idx, "train": np.concatenate(split['train'][1])}
    query_idx = []
    count = 0
    ...
    fs_loader = SimpleFSManager(class_ind=split[split_name][0], data_ind=query_idx, k_shot=0, q_query=1, n_way=kwargs["n_way"],
                                min_n_way=kwargs.get("min_n_way"), task_level=task_level,)

    
    
    dataset_ori = FewShotDataset_fixed_hard(fs_loader, query_graph_dataset, support_graph_dataset,
                          prompt_feats["prompt_edge_text_feat"][-2:], task_level, fs_sample_size)
...
class UnifiedTaskConstructor:
    def __init__(self, tasks: list[str], load_texts: bool, encoder: utils.SentenceEncoder,
                 ...
                 num_epochs=30, prune_ratio=0.5, delta=0.875, fs_sample_size=2000, hard_pruning_ratio=0.0, hard_pruning_epochs=25):
        """
    ...
    def add_dataset(self, stage_config, dataset_config):
        ...
        if stage_config["stage"] == "train" and dataset_config["construct"] == "ConstructFSTask":
            data = globals()[dataset_config["construct"] + "_fixed_hard"](dataset=data, split=split, split_name=stage_config["split_name"],
                    ...
                    dataset_config["process_label_func"]] if dataset_config.get("process_label_func") else None,
                                                        ...
                                                        fs_sample_size=self.fs_sample_size,
                                                        **dataset_config["args"], )
        else: 
    ...
    def make_train_data(self):
        ...
        train_data = InfoBatchMultiDataset(self.datasets["train"], num_epochs=self.num_epochs, prune_ratio=self.prune_ratio, delta=self.delta, hard_pruning_ratio=self.hard_pruning_ratio, hard_pruning_epochs=self.hard_pruning_epochs)
        return train_data
    
    def make_full_dm_list(self, train_data=None):
        text_dataset = {
            "train": DataWithMeta(self.make_train_data() if not train_data else train_data,
