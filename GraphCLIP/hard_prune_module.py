...
def hard_prune_api(data_list, batch_size=32, weight_decay=5e-4, nlayer=5, max_epochs=25, devices=[0]):
    ...
    model = MyGIN(dataset[0].x.shape[1], weight_decay=weight_decay, nlayer=nlayer)
    ...
    if trainer.world_size > 1:
        ...
        if trainer.is_global_zero:  
            infomative_scores = torch.cat([scores for scores in gathered_scores])
            return infomative_scores.cpu().detach().numpy()
    else:
...
if __name__ == '__main__':
    ...
    print(tmp_data_list)    
    infomative_scores = hard_prune_api(tmp_data_list, batch_size=32, weight_decay=5e-4, nlayer=5, devices=[0])
    print(infomative_scores)
